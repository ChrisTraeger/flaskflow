...
import { Task, COLUMNS, PRIORITY_META, CreateTaskDTO, UpdateTaskDTO, Priority } from "./domain/types";
...
function Board({ onLogout }: { onLogout: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    await createTask({ title: form.title.trim(), description: form.description, project_id: PROJECT_ID, priority: form.priority, assignee_id: form.assignee_id || undefined, due_date: form.due_date || null });
    setForm(EMPTY_FORM);
    setShowCreate(false);
  };
  ...
  const handleUpdate = async () => {
    if (!editTask) return;
    await updateTask(editTask.id, { title: editForm.title.trim(), description: editForm.description, priority: editForm.priority, due_date: editForm.due_date || null });
    setEditTask(null);
  };
  ...
  return (
    <div style={s.app}>
      ...
      {showCreate && <TaskFormModal title="Nueva tarea" form={form} onChange={setForm} onConfirm={handleCreate} onCancel={() => { setShowCreate(false); setForm(EMPTY_FORM); }} confirmLabel="Crear tarea" />}
      {editTask && <TaskFormModal title={`Editar: ${editTask.title}`} form={editForm} onChange={setEditForm} onConfirm={handleUpdate} onCancel={() => setEditTask(null)} confirmLabel="Guardar cambios" />}
    </div>
  );
}
...
// ── TaskFormModal ─────────────────────────────────────────────────────────────
interface TaskFormModalProps { title: string; form: TaskFormState; onChange: (f: TaskFormState) => void; onConfirm: () => void; onCancel: () => void; confirmLabel: string; }

function TaskFormModal({ title, form, onChange, onConfirm, onCancel, confirmLabel }: TaskFormModalProps) {
  const set = (key: keyof TaskFormState, val: string | number) =>
    onChange({ ...form, [key]: key === "priority" ? Number(val) : String(val) } as TaskFormState);
  return (
    <div style={s.modal} onClick={e => e.target === e.currentTarget && onCancel()}>
      <div style={s.modalBox}>
        ...
        <select style={s.input} value={form.priority} onChange={e => set("priority", Number(e.target.value))}>
          <option value={1}>Baja</option><option value={2}>Media</option>
          <option value={3}>Alta</option><option value={4}>Urgente</option>
        </select>
